...
package com.zhongdasoft.svwtrainnet.widget.zxingcamera;
    ...
    void setDesiredCameraParameters(Camera camera) {
        ...
        if (supportedPreviewSizes.size() > 0) {
            ...
            if (cameraResolution.x > cameraResolution.y) {
                currRatio = cameraResolution.y * 1.0f / cameraResolution.x;
            } else {
                currRatio = cameraResolution.x * 1.0f / cameraResolution.y;
            }
            float supportRatio;
            for (Size size : supportedPreviewSizes) {
                supportRatio = size.height * 1.0f / size.width;
                if (Math.abs(supportRatio - currRatio) < ratio) {
                    ratio = Math.abs(supportRatio - currRatio);
                    position = i;
                }
                i++;
            }
