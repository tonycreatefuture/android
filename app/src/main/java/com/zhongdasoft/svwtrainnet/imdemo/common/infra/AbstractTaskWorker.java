...
public abstract class AbstractTaskWorker {	
	...
	private final Executor getExecutor(Task task) {
		...
		return com.zhongdasoft.svwtrainnet.imdemo.common.infra.TaskExecutor.IMMEDIATE_EXECUTOR;
	}
	
	private final Runnable getRunnable(final com.zhongdasoft.svwtrainnet.imdemo.common.infra.Task task) {
		return new Runnable() {
			@Override
			public void run() {
				// schedule
