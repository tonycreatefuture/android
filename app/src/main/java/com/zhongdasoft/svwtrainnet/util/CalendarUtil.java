...
public class CalendarUtil {
     ...
     * @return 农历的日期
     */
    public String toLunarString(int y, int m, int d) {
        String message;
        // int n = mLuchDay % 10 == 0 ? 9 : mLuchDay % 10 - 1;
        this.setGregorian(y, m, d);
        this.computeChineseFields();
        this.computeSolarTerms();
        message = getChinaCalendarMsg(chineseYear, chineseMonth, chineseDate);
        if (StringUtil.isNullOrEmpty(message)) {
            String solarMsg = getSolartermsMsg();
            // 判断当前日期是否为节气
            if (!StringUtil.isNullOrEmpty(solarMsg)) {
                 ...
                 * 判断当前日期是否为公历节日
                 */
                String greMessage = new GregorianUtil(m, d)
                        .getGremessage();
                if (!StringUtil.isNullOrEmpty(greMessage)) {
                    message = greMessage;
                } else if (chineseDate == 1) {
                    message = chineseMonthNames[chineseMonth < 0 ? -1 * chineseMonth : chineseMonth - 1] + "月";
                } else {
                    int cd = this.getChineseDate();
                    message = daysOfAlmanac[cd - 1];
                }
    ...
    private String getChinaCalendarMsg(int year, int month, int day) {
        ...
        if (((month) == 1) && day == 1) {
            ...
            if (month == 12) {
                if ((((daysInChineseMonth(year, month) == 29) && day == 29))
                        || ((((daysInChineseMonth(year, month) == 30) && day == 30)))) {
                    message = "除夕";
                }
            }
        }
    ...
    public static String getDate(String startTime, String finishTime, String scheduleType) {
        if ("NewOnlineCourse".equals(scheduleType)) {
            return "日期：" + startTime.substring(0, 10);
        } else {
            if ("CounselingTraining".equals(scheduleType) || "InternalTraining".equals(scheduleType)) {
                if (StringUtil.isNullOrEmpty(finishTime)) {
                    return "日期：" + startTime.substring(0, 10);
                } else {
                    return "日期：" + startTime.substring(0, 10) + " 至 " + finishTime.substring(0, 10);
                }
            } else if ("InternalTrainingExpire".equals(scheduleType)) {
                return "日期：" + startTime.substring(0, 10);
            } else {
                if (StringUtil.isNullOrEmpty(finishTime)) {
                    return "日期：" + startTime.replace("T", " ");
                } else {
                    return "日期：" + startTime.replace("T", " ") + " 至 " + finishTime.replace("T", " ");
                }
            }
        }
    }

    public static String getPlaceName(String place, String scheduleType) {
        if (!"OnlineExam".equals(scheduleType)) {
            return "地点：" + place;
        }
        String[] placeArr = {"1", "2", "3"};
        String[] placeNameArr = {"网页", "APP", "网页 APP"};
        for (int i = 0; i < placeArr.length; i++) {
            if (place.equals(placeArr[i])) {
                return "考试方式：" + placeNameArr[i];
            }
        }
        return "";
    }
    ...
    public static String getTypeName(String typeCode) {
        String[] typeCodeArr = {"OfflineTraining", "CounselingTraining", "InternalTraining", "InternalTrainingExpire", "OnlineExam", "NewOnlineCourse", "MeetingPlan", "MoocApplyExpire"};
        String[] typeNameArr = {"线下培训安排", "辅导培训安排", "经销商内部培训安排", "总部内训截止时间", "在线考试安排", "新加入的在线课程", "网上直播安排", "慕课堂报名截止时间"};
        for (int i = 0; i < typeCodeArr.length; i++) {
            if (typeCode.equals(typeCodeArr[i])) {
                return typeNameArr[i];
            }
        }
        return "";
    }
